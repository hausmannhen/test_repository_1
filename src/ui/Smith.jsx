...
export default function Smith({ G, onClose, rerender }) {
  ...
  return (
    <Panel title="Schmied" gold={P.gold} footer={<Btn tone="gold" onClick={onClose}>Verlassen</Btn>}>
      ...
      {sel && <ItemDetail P={P} item={sel} actions={[
        <Btn key="u" small tone="gold" disabled={sel.upg >= 5 || P.gold < upgradeCost(sel)} onClick={() => { upgradeItem(P, sel); rerender(); }}>
          {sel.upg >= 5 ? "Maximal verstärkt" : `Aufwerten für ${upgradeCost(sel)} Gold`}
        </Btn>]} />}
      {gear.length === 0 && <div className="empty">Du trägst nichts, das ich schmieden könnte.</div>}
      {gear.map(it => <ItemRow key={it.uid} item={it} selected={sel === it} onClick={() => setSelected(it)} right={<span className="dim" style={{ fontSize: 12 }}>{P.equip[it.slot] === it ? "angelegt" : ""}</span>} />)}
    </Panel>
  );
}
